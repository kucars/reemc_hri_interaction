...
void get_frame(const sensor_msgs::Image& img)
{
    ...
    cvImgPtr = cv_bridge::toCvCopy(img,enc::TYPE_8UC1);
...
{
	init(argc, argv, "debug_node2");
	double val;
	namedWindow("frame2",cv::WINDOW_AUTOSIZE);
	...
	while (ros::ok())
	{
        ...
        {
        	//draw_loc();
        	rectangle(frame, Point(frame.size().width/2 - 10,frame.size().height/2 -10), Point(frame.size().width/2 + 10,frame.size().height/2 +10), CV_RGB(0,255,0), 2);
        	cout<<(float)frame.at<uchar>(frame.size().height/2,frame.size().width/2)<<endl;
        	//val=(6000*val)/255.0;
            imshow("frame",frame);
            waitKey(30);
        }
