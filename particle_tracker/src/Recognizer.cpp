...
bool Recognizer::prepare(string a)
{
    ...
    int prev_class;
    fn_csv=a;
    db_abs_path=fn_csv.substr(0,fn_csv.rfind("/")+1);
    ...
    while (getline(file, line)) {
        ...
        if(!path.empty() && !classlabel.empty()) {
            temp=imread(db_abs_path+path, 0);
            if(temp.empty())
                continue;
   ...
   string temp=path.erase(path.rfind("/"));
   db_rel_path=temp.erase(temp.rfind("/"));
   db_abs_path+=db_rel_path;
   cout<<db_abs_path<<endl;
   cout<<db_rel_path<<endl;
...
{
    string path="/s"+ToString(class_count+1);
    vector<int> params;
    vector<int> label;
    ...
    string command="mkdir "+db_abs_path+path;
    system(command.c_str());
    for(int i=0;i<imgs.size();i++)
    {
        resize(imgs[i],imgs[i],im_size);
        cvtColor(imgs[i],imgs[i],CV_BGR2GRAY);
        imwrite(db_abs_path+path+"/"+ToString(i+1)+".pgm",imgs[i],params);
        outfile<<db_rel_path+path+"/"+ToString(i+1)+".pgm;"<<class_count<<"\n";
        label.push_back(class_count);
    }
    model->update(imgs,label);
    class_count++;
}
